...
decode_table::decode_table(vector<code_size> measure, deque<string> symbol) : pos(0), table(){
	size_t total = 0;
	for(code_size &ob:measure){
		total += ob.count;
		assert(ob.length=<64);
	}
...
bool decode_table::check_code(const uint64_t &in){
	uint64_t diff = in - table[pos].start;
	return (diff<table[pos].count&&diff>=0);
}
string decode_table::get_string(const uint64_t &in){
	size_t diff = in - table[pos].start;
	assert(diff<table.symbols.size());
	return table.symbols[diff];
}
size_t decode_table::next(){
	size_t counter = table[pos].count;
	assert((pos+1)<table.size());
	pos++;
	return(table[pos].size-counter);
}
void decode_table::reset(){
	pos = 0;
